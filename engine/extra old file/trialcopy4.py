...
def analyse_lookahead(
    ...
    """
    Check today's workforce plus the next PLANNING_HORIZON days.

    Existing scheduled hires are included in the simulation.
    This function never creates a real hire.
    """
    simulated_staff = int(current_staff)
    simulated_queue = clone_pending_hires(pending_hires)
    ...
    for index in range(current_index, end_index + 1):
        date = pd.to_datetime(dataframe.at[index, "Date"])
        ...
        required = int(
            dataframe.at[index, "RequiredWorkers"]
        )
        ...
        dates.append(date)
        required_workers.append(required)
        available_workers.append(simulated_staff)
        gaps.append(gap)

    positive_runs = find_positive_gap_runs(gaps)
    severe_runs = find_severe_gap_runs(gaps)
    ...
    return {
        ...
        "DailyGaps": gaps,
        "GapDays": int(sum(gap > 0 for gap in gaps)),
        "TotalGap": int(sum(gaps)),
        "MaximumGap": int(max(gaps, default=0)),
        "AverageGap": round(
            float(np.mean([gap for gap in gaps if gap > 0]))
            if any(gaps)
            else 0.0,
            2,
        ),
...
for index in range(len(df)):
    ...
    # Employees hired 7 days earlier become available today.
...
decision_assumptions = pd.DataFrame(
    {
        ...
        "Value": [
            ...
            "Required workers x overtime hours x hourly rate x 1.5",
            WORKING_HOURS_PER_DAY,
            MAX_OVERTIME_PER_WORKER,
            OVERTIME_MULTIPLIER,
            "Scheduled hires are included in future staff; duplicate hires are prevented",
        ],
    }
)
